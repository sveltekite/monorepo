...
import { SchemaParser, type Schema, type EntityConfig } from './schema-parser.js'

export class CodeGenerator {
  constructor(private schema: Schema, private projectPath: string) {}
    ...
    const joinTableSchemas = Object.entries(this.schema.joinTables)
      .map(([tableName, config]) => {
        const zodFields = Object.entries(config.fields).map(([fieldName, fieldConfig]) => {
          return `   ${fieldName}: z.string().uuid()`
        }).join(',\n')
...
`
  ...
  async generateEntityClass(entityName: string, config: EntityConfig): Promise<void> {
    ...
    // Generate imports
    const imports = `import type { ${schemaType} } from '../schema.js'
...
`
  ...
  private generateRelationMethods(entityName: string, relations: Record<string, any>): string {
  ...
  async copyRouteTemplates(): Promise<void> {
...
`
  ...
  static async fromYamlFile(yamlPath: string, projectPath: string): Promise<CodeGenerator> {
